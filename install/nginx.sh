...
public_file=/RustPanel/install/public.sh
...
System_Lib() {
	...
	nohup bash fix_install.sh > /RustPanel/install/fix.log 2>&1 &
...
Install_Configure() {
    ...
    [ -f "/RustPanel/install/nginx_prepare.sh" ] && . /RustPanel/install/nginx_prepare.sh
    [ -f "/RustPanel/install/nginx_configure.pl" ] && ADD_EXTENSION=$(cat /RustPanel/install/nginx_configure.pl)
...
Install_Nginx() {
	...
	md5sum ${Setup_Path}/sbin/nginx > /RustPanel/data/nginx_md5.pl
...
Set_Conf() {
    ...
    mkdir -p /RustPanel/vhost/nginx/tcp
...
EOF
    ...
    mkdir -p /RustPanel/vhost
    #wget -O waf.zip ${download_Url}/install/waf/waf.zip
    #unzip -o waf.zip -d $Setup_Path/ >/dev/null
    if [ ! -d "/RustPanel/vhost/wafconf" ]; then
        mv $Setup_Path/waf/wafconf /RustPanel/vhost/wafconf
    fi

    sed -i "s#include vhost/\*.conf;#include /RustPanel/vhost/nginx/\*.conf;#" ${Setup_Path}/conf/nginx.conf
    ...
    AA_PANEL_CHECK=$(cat /RustPanel/config/config.json | grep "English")
    if [ "${AA_PANEL_CHECK}" ]; then
        #\cp -rf /RustPanel/data/empty.html /www/server/nginx/html/index.html
        ...
        wget -O /RustPanel/vhost/nginx/0.default.conf ${download_Url}/conf/nginx/en.0.default.conf
