...
Install_Curl()
{
	if [ ! -f "/usr/local/curl/bin/curl" ];then
		...
		cd curl-${curlVersion}
		./configure --prefix=/usr/local/curl --enable-ares --without-nss --with-ssl=/usr/local/openssl --build=arm-linux
		make -j${cpuCore}
		make install
		cd ..
...
Install_Freetype()
{
	if [ ! -f "/usr/bin/freetype-config" ] && [ ! -f "/usr/local/freetype/bin/freetype-config" ]; then
		...
		./configure --prefix=/usr/local/freetype --enable-freetype-config --build=arm-linux
		make -j${cpuCore}
		make install
		cd ../
...
Install_Libiconv()
{
	...
	echo -e "Install_Libiconv" >> /RustPanel/lib.pl
}
Install_Libmcrypt()
{
	...
	echo -e "Install_Libmcrypt" >> /RustPanel/lib.pl
}
Install_Mcrypt()
{
	...
	echo -e "Install_Mcrypt" >> /RustPanel/lib.pl
}
Install_Mhash()
{
    ...
    cd ${run_path}
    rm -rf mhash-0.9.9.9*
	echo -e "Install_Mhash" >> /RustPanel/lib.pl
}
